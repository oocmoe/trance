...
const PromptContentList = () => {
  ...
  const [, setActionPromptContentModal] = useAtom(modalAtom('actionPromptContent'));
...
const PromptContentSwitch = ({
  ...
  const handleUpdate = async () => {
    const result = await updatePromptContentField(id, contentId, 'isEnabled', !isEnabled);
    if (!result) {
      toast.error('更改失败');
    }
  };
...
const PromptContetnModal = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updatePromptContentField(Number(id), actionContentId, 'content', content);
      setIsOpen(false);
      toast.success('修改成功');
    } catch (error) {
      console.log(error);
